...
int pointSize = 50;
float highY = 100, tight = 0.2;
int margin = 10;
int gap = 100, pointer = 0;
int yGap = 5;
int rows, cols;
...
int fromOrigin() {
  return height - margin - (height/2);
}

int fromOrigin(int y) {
  return height - margin - y - int(highY/2);
}

float fromOrigin(float y) {
  return height - margin - y - (highY/2);
}
